...
#music
pygame.mixer.music.load('music/NEWTONINSIDE.mp3')
...
class npc(pygame.sprite.Sprite):
    def __init__(self, char_type, x, y, scale, format):
        pygame.sprite.Sprite.__init__(self)
        self.char_type = char_type
        self.flip = False
        self.animation_list = []
        ...
        for animation in animation_types:
            ...
            temp_list = []
            # count number of files in the folder
            num_of_frames = len(os.listdir(f'npc/{self.char_type}/{animation}'))
            for i in range(num_of_frames):
                img = pygame.image.load(f'npc/{self.char_type}/{animation}/{i}.{self.format}')
                img = pygame.transform.scale(img, (int(img.get_width() * scale), int(img.get_height() * scale)))
                temp_list.append(img)
            self.animation_list.append(temp_list)
        self.image = self.animation_list[self.action][self.frame_index]
...
coins_group.add(coins)
npcs = npc('NEWTON NPC', 460, 150, 0.5, 'png')
npc_group.add(npcs)
...
AUDIOIMG = pygame.image.load('img/newtondiag.png')
...
def dialog(str,speed,pos,color):
    ...
    while len(text) < len(str):
        ...
        clock.tick(speed)
counter_dialog = 0
dialog1 = pygame.mixer.Sound('music/AUDIOS FINAL/NEWTON LADRON.mp3')
...
while run:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_x and pygame.sprite.groupcollide(player_group, npc_group, False, True):
                if counter_dialog == 0:
                    pygame.mixer.music.set_volume(0.1)
                    screen.blit(AUDIOIMG, (0, 0))
                    dialog1.play()
                    dialog('ERA CUESTION DE TIEMPO DE QUE ALGUIEN', 13, (300, 450), (255, 255, 255))
                    dialog('INTENTARA ROBAR MIS APUNTES, NO TE LA ', 13, (300, 500), (255, 255, 255))
                    dialog('PONDRE NADA FACIL', 13, (300, 550), (255, 255, 255))

                    counter_dialog += 1
                time.sleep(1)
                pygame.quit()
                subprocess.call(["python", "BOSS BATTLE 1.py"])
